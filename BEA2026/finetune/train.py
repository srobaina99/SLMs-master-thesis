...
def build_enriched_input(row, sep_token: str = "[SEP]") -> str:
...
def prepare_dataset(df: pd.DataFrame, sep_token: str = "[SEP]", target_mean: float = None, target_std: float = None):
    ...
    """
    inputs = df.apply(lambda row: build_enriched_input(row, sep_token=sep_token), axis=1).tolist()
...
def main():
    ...
    if not args.no_drive:
        ...
        if drive_path.exists():
            ...
            args.output_dir = str(drive_base / "output")
    ...
    report_lines = [
        ...
        f"**Input format:** `wlen={{N}} | nedit={{N}} | pos={{POS}} | clue={{N}} | L1_word [SEP] L1_context [SEP] clue [SEP] en_word`",
        "",
        "## Results",
        "",
    ...
    report_lines += [
        ...
        "## Comparison",
        "",
        "| Model | ES RMSE | ES Pearson |",
        "|---|---|---|",
        "| XLM-R baseline (closed) | 1.357 | 0.748 |",
        "| full_xgb (feature best) | 1.464 | 0.646 |",
    ]

    # Add ensemble row if available
    ens_path = pred_dir / "closed" / "dev" / "es" / "mdeberta_ensemble_preds.csv"
    if ens_path.exists():
        ens_df = pd.read_csv(ens_path)
        if "es" in datasets_cache:
            eval_df = datasets_cache["es"][2]
            merged = eval_df[["item_id", "GLMM_score"]].merge(ens_df, on="item_id")
            rmse = root_mean_squared_error(merged["GLMM_score"].values, merged["prediction"].values)
            r, _ = pearsonr(merged["prediction"].values, merged["GLMM_score"].values)
            report_lines.append(f"| **mdeberta_ensemble** | **{rmse:.3f}** | **{r:.3f}** |")
